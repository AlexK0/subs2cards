...
class TranslateDialog(QDialog):
    ...
    def __init__(self, parent: QDialog, shared_token: SharedTranslatedToken):
        ...
        self._ok_button = make_button(self, "OK", 50, 980, self._web.height() + 20, self.accept)
        ...
        if self._shared_token.native_translations:
            self._translation.setText(", ".join(self._shared_token.native_translations))

        self._on_translation_enter()
        self._show_translation(self._GOOGLE_TRANSLATE)

    def _show_translation(self, page: str) -> None:
        self._web.load(QUrl(self._TRANSLATE_PAGES[page] % self._shared_token.token.word))
        self._web.show()
